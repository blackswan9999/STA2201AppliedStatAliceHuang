...
```{r}
dm %>% group_by(Age) %>%
  summarize(mean_female = mean(Female, na.rm=TRUE), mean_male = mean(Male, na.rm=TRUE))
```
...
```{r}
dm %>% group_by(Age) %>%
  summarize(across(c("Male", "Female"), mean))
```
...
dm_long %>% pivot_wider(names_from = sex, values_from = "mortality") -> dm
dm
```
...
```{r}
p <- ggplot(data = d_to_plot, aes(x = Year, y = Male)) + geom_line()
p
```
...
```{r}
p <- ggplot(data = d_to_plot, aes(x = Year, y = Male)) + geom_line(color = "firebrick4") + labs(title = "Male Mortality Rates over time, Ontario", y = "Mortality Rate")
p
```
...
dm %>% filter(Age == 30) %>%
  select(Year:Male) %>%
  pivot_longer(Female:Male, names_to = "Sex", values_to = "Mortality") -> dp
```
...
```{r}
dp %>% ggplot(aes(x=Year, y=Mortality, color = Sex)) + geom_line() + scale_color_brewer(palette="Set1")
```
...
```{r}
dp %>% ggplot(aes(x=Year, y=Mortality, color = Age)) + geom_line() + facet_grid(~Sex)
```
...
```{r}
dm %>% filter(Age == 10 | Age == 20| Age == 30 | Age == 40)%>%
  mutate(mf_ratio = Male/Female) %>%
  ggplot(aes(x=Year, y= mf_ratio, col=Age)) + geom_line() + theme_bw() + labs(y="ratio of male to female mortality rates")
```
...
```{r}
dm %>% select(Year, Age, Female) %>%
  group_by(Year) %>%
  # subtract 1 since age starts at 0, and R start indexing at 1
  summarize(m = which.max(Female)-1)
```
...
```{r}
dm %>% summarize(across(c("Male", "Female", "Total"), sd, na.rm=TRUE))
```

The standard deviations of mortality rates for the Male, Female and Total populations are respectively 0.3327571,	0.3111401	and 0.2960847.	 
...
pop_dm <- left_join(pop, dm, by=c("Year", "Age"), suffix = c("_pop", "_mort"))

head(pop_dm)
```

```{r}
pop_dm %>% mutate(Female_popweight_mort = (Female_pop/Total_pop)*Female_mort) %>% 
  mutate(Male_popweight_mort = (Male_pop/Total_pop)*Male_mort) %>%  
  group_by(Year) %>% 
  summarize(female_avgweighted_pop_mort = sum(Female_popweight_mort, na.rm=TRUE),
            male_avgweighted_pop_mort = sum(Male_popweight_mort, na.rm=TRUE)) -> newpop

head(newpop)
```
```{r}
newpop %>% pivot_longer(c(female_avgweighted_pop_mort, male_avgweighted_pop_mort), names_to = "sex", values_to = "mortality") %>% 
  ggplot(aes(x=Year, y=mortality, color = sex)) + geom_line() + theme_bw() + labs(y="population weighted average mortality")
```

From 1975-2020, it looks like the gap between the Female and Male average population-weighted mortality is increasing. The male average population-weighted mortality seems to show a decreasing trend, but the same cannot be said for the female average population-weighted mortality.