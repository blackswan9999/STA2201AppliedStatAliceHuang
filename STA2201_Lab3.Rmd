...
This assumes that $\alpha + 1 = 10 , \beta + 1 = 10$. we assume that there are 9 successes and 9 failures. This is more information than the prior in Question 2. The prior in Question 2 is equivalent to Unif(0,1) prior which assumes that everything in (0,1) is equally likely. However with a Beta(10,10) prior, you assume that some values are more likely than others, which is extra information.  
...
ggplot(df, aes(x_theta, y_lik)) + geom_line() + stat_function(fun="rbeta", args = list(129, 1,1), col="red") + stat_function(fun=rbeta, args = list(129, 10, 10), col="blue") + stat_function(fun="rbeta", args = list(529, 119, 12))
```
