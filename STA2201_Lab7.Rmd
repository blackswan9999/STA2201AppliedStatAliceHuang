...
Look at 3 models for $\log(\theta_i)$
...
stan_data <- list(y = observe.i, log_e = log(expect.i), N=length(observe.i), x=aff.i-mean(aff.i))
```
```{r}
mod1 <- stan(data=stan_data, file = "lab9_1.stan")
```
```{r}
mod1
```
...
```{r}
mod2 <- stan(data=stan_data, file = "lab9_2.stan")
```

### Model 3

$$\log \theta_i = \alpha_i + \beta x_i$$ with

$$\alpha_i \sim N(\mu, \sigma^2)$$

```{r}
mod3 <- stan(data=stan_data, file = "lab9_3.stan")
```
...
```{r}
mod1 %>% gather_draws(log_theta[i]) %>%
  median_qi() %>%
  rename(median_mod1 = .value, lower_mod1 = .lower, upper_mod1 = .upper) %>%
  select(i, median_mod1:upper_mod1) -> res_mod1

mod2 %>% gather_draws(log_theta[i]) %>%
  median_qi() %>%
  rename(median_mod2 = .value, lower_mod2 = .lower, upper_mod2 = .upper) %>%
  select(i, median_mod2:upper_mod2) -> res_mod2


mod3 %>% gather_draws(log_theta[i]) %>%
  median_qi() %>%
  rename(median_mod3 = .value, lower_mod3 = .lower, upper_mod3 = .upper) %>%
  select(i, median_mod3:upper_mod3) -> res_mod3
...
```{r}
res %>% select(median_mod1, median_mod2, median_mod3) %>%
  pivot_longer(meidan_mod1:median_mod3, names_to = "model", values_to = "log_theta") %>%
  mutate(model = str_remove(model, "median")) %>%
  ggplot(aes(log_theta, fill = model)) + geom_histogram(position = "dodge")
```

```{r}
res %>% mutate(deaths = observe.i) %>%
  mutate(log_smr = log(observe.i/expect.i)) %>%
  ggplot(aes(log_smr, median_mod1, colour = "Model 1")) + 
  geom_point(size = deaths, alpha = 0.6) +
...
Model 2 and Model 3 look similar, but there is more uncertainty around Model 2 (bias variance tradeoff). 