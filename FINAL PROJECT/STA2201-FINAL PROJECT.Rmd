...
output:
  ...
  html_document:
    df_print: paged
abstract: We investigate the effects of water and nitrogen levels on maize yields
  across different soil types in China. We fit a Bayesian hierarchical model with
  a Normal likelihood on maize yield, grouping by soil type and investigating the
  effects of water and nitrogen within each soil type. We show that increasing water
  and nitrogen results in greater increases in maize yields for coarser soil types
  that are worse at retaining water and nutrients.
---
...
```{r}
# read in dataset
datamaize <- read_csv("https://raw.githubusercontent.com/blackswan9999/STA2201AppliedStatAliceHuang/main/Book2.csv")
datamaize_readme <- read_xlsx("Dataset instrumetn.xlsx")

# get only observations in China, select variables we're interested in
datamaizechina <- datamaize %>% filter(Region == "China") %>%
  dplyr::select(Yield, MAT, Texture, PPH, SOC, Water, Nitrogen)

# filter out rows with missing Yield
datamaizechina %>% filter(!is.na(Yield)) -> datamaizechina

# make sure text entries for soil types are consistent
...
# filter out rows with missing soil information
...
library(ggrepel)
# create dataframe with proportions of soil types in dataset
datamaizechina %>% group_by(Texture) %>%
  summarize(prop = n()/nrow(datamaizechina)) -> dfpie
# make a pie chart for proportions of soil types in dataset
...
set.seed(777)
y <- maize
# extract simulated maize yield estimates under model
yrep1 <- extract(mod)[["y_hat"]]
# randomly sample 100 of those estimates
samp100 <- sample(nrow(yrep1), 100)
# plot the 100 maize yield estimates' densities 
# along with the observed maize yield estimate's density  
plot_ys <- ppc_dens_overlay(y, yrep1[samp100, ])
# plot the simulated median maize estimates
# along with the observed median maize yield from dataset
plot_meds <- ppc_stat(y, yrep1, stat = 'median')
# arrange two graphs side by side
...
maize2 <- box(datamaizechina2$Yield, bestlambda)
