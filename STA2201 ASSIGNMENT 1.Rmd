...
Then since $\theta$ is Gamma distributed $\pi(\theta) = \frac{1}{\Gamma(\alpha)\beta^\alpha}\theta^{\alpha-1}e^{-\theta/\beta}$ and since $x|\theta$ is Poisson distributed, $L(x|\theta) =\frac{(\mu\theta)^x}{x!}e^{-\mu\theta}$
...
Here $p = \frac{1}{\beta \mu + 1}, k=y, r=\alpha$.
...
Inspired by the shape of the histogram for the death toll count, we also considered using a Poisson regression to model death toll counts. To solve the issue of counts not giving an accurate perception due to not reflecting the underlying population, we offset by the population variable. Since the response is being modeled using counts, the explanatory variables should also be modeled using counts. Then we checked for overdispersion, since using a Poisson regression assumes that the mean and variance are the same.
...
There seemed to be a lot of overdispersion (1005.821>>1) so a Poisson regression is not appropriate as the mean and variance would not be the same. 
...
Unfortunately, the quasipoisson model seems to have a very high residual deviance given the number of residual degrees of freedom. So we consider a negative binomial model.

The negative binomial model is similar to the Poisson regression model but it does not require the mean and variance to be the same. We also include an offset term for the population in the negative binomial model. This requires assuming that the death count conditioned on the parameters follows a Poisson distribution and that the parameter space follows a Gamma distribution. Poisson distribution 
...
The negative binomial model on the counts seems to have a more reasonable Residual Deviance given the number of degrees of freedom. We also note that the residuals of the negative binomial model have a 0 and the variance is closest to 1. The residuals of the quasipoisson model have much larger variance. Thus we choose the negative binomial model. 

```{r echo=FALSE}
plot(residuals(quasipoisreg))
plot(residuals(negbinvax))
```
...
For the sake of comparison, I also tried using the quasi-poisson model for prediction and that yielded around 74%. However, I will stick with the negative binomial model because I think the residual fit is better overall.

The negative binomial regression model assumes that the adult vaccination count conditioned on the parameters follows a Poisson distribution. In reality, it is difficult to satisfy the assumption that the adult vaccination count conditioned on the parameters follows a Poisson distribution. That would require that the rate at which adults get vaccinated is constant. The average rate at which adults get vaccinated in a given time period may change depending on vaccine availability and (mis)information about vaccines in the media. Also, people are fully vaccinated, they don't become fully vaccinated again. It is difficult to satisfy the assumption that people got vaccinated independently of each other, especially when workplaces and public places like restaurants and event venues mandated them. Furthermore, people are probably more likely to get vaccinated if their friends and family are also getting vaccinated and encouraging them to do so.
