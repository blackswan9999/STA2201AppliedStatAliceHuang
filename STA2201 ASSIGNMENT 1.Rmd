...
f(y)&= \binom{y+\alpha-1}{y} (\frac{\beta \mu}{\beta \mu + 1})^{y} (1- \frac{\beta \mu}{\beta \mu + 1})^{\alpha}
\end{align*}

The Negative Binomial probability mass function is $f(k) = \binom{k+r-1}{k} (1-p)^k p^r$

Here $p = 1-\frac{\beta \mu}{\beta \mu + 1}, k=y, r=\alpha$.
...
We also considered using a Poisson regression to model count data, inspired by the shape of the histogram for the death toll. To solve the issue of counts not giving an accurate perception due to not reflecting the underlying population, we offset by the population variable. Since the response is being modeled using counts, the explanatory variables should also be modeled using counts. Then we checked for overdispersion.
