...
It seems that there is weakly positive correlation between the mother's IQ score and the kid's IQ score.
...
There seems to be strong correlation between alpha and beta, the slope and the intercept. The marginal distribution of alpha follows a symmetric bell shaped distribution. The marginal distribution of beta also follows a symmetric bell shaped distribution. 
...
fit4 %>%
  spread_draws(alpha, beta[k], sigma) %>% 
  # only takes the variable for mom's high school education
  filter(k==1) %>%
  # add effect of mom's iq being 110
     mutate(nhs = alpha + 0.57*(110 - mean(kidiq$mom_iq)), # no high school is just the intercept
          hs = alpha + beta + 0.57*(110 - mean(kidiq$mom_iq))) %>% 
  select(nhs, hs) %>% 
  pivot_longer(nhs:hs, names_to = "education", values_to = "estimated_score") %>% 
  ggplot(aes(y = education, x = estimated_score)) +
  stat_halfeye() + 
...
data.frame(ppd) %>% ggplot(aes(x=ppd)) + geom_histogram()
```
